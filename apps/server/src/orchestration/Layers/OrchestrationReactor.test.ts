...
import { SourceTruthPreflightReactor } from "../Services/SourceTruthPreflightReactor.ts";
  ...
  it("starts provider ingestion, provider command, checkpoint, and thread deletion reactors", async () => {
    ...
    runtime = ManagedRuntime.make(
      Layer.effect(OrchestrationReactor, makeOrchestrationReactor).pipe(
        ...
        Layer.provideMerge(
          Layer.succeed(SourceTruthPreflightReactor, {
            start: () => {
              started.push("source-truth-preflight-reactor");
              return Effect.void;
            },
          }),
        ),
    ...
    expect(started).toEqual([
      ...
      "source-truth-preflight-reactor",

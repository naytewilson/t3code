...
export const make = Effect.gen(function* () {
  ...
  return DesktopUpdates.of({
    ...
    configure: Effect.gen(function* () {
      ...
      // Must stay true on macOS: electron-updater only feeds the downloaded
      // zip to the native Squirrel.Mac stager when autoInstallOnAppQuit is
      // true. With false, the manual install path (quitAndInstall) has
      // nothing staged and fails, and quitting never installs either. The
      // product-level confirm dialog still gates quitAndInstall; this flag
      // additionally means an ordinary quit installs an already-downloaded
      // update, which is the intended seamless-update behavior.

...
export const CommandCodeSettings = makeProviderSettingsSchema(
  {
    // Off by default (like Cursor, Grok, OpenCode): users opt in from Settings.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("commandcode-acp").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the commandcode-acp ACP bridge binary.",
        providerSettingsForm: { placeholder: "commandcode-acp", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
